...
async def home():
    html_content = """
        ...
        <body>
            <div class="container">
                <div class="header">
                    <h1>CVE/BDU Загрузка в базу данных</h1>
                    <p>Выберите файлы CVE и/или BDU для загрузки в MongoDB</p>
                    <div style="margin-top: 10px;">
                        <button class="reload-btn" onclick="refreshPage()">🔄 Обновить страницу</button>
                    ...
                    <div class="card">
                        ...
                        <div class="card-body">
                            ...
                            <div class="file-selector" id="fileSelector" onclick="selectFiles()">
                                ...
                                <div class="subtext">или перетащите файлы сюда (поддерживается множественный выбор)</div>
                            ...
                            <div class="button-group">
                                <button class="btn-small danger" onclick="clearAllFiles()">🗑️ Очистить</button>
                                <button class="btn-small" onclick="autoDetectFiles()">🔍 Автоопределение</button>
                            </div>
            ...
            <script>
                ...
                function refreshPage() {
                    // Просто перезагружаем страницу GET запросом
                    window.location.href = window.location.pathname;
                }

                function clearAndRefresh() {
                    // Очищаем сохраненный taskId и перезагружаем
                ...
                // ----- РАБОТА С LOCALSTORAGE (только для taskId) -----
                ...
                function autoDetectFiles() {
                    ...
                    if (newCve) {
                        cveFile = newCve;
                        document.getElementById('cveFileName').textContent = cveFile.name;
                    }
                    
                    if (newBdu) {
                        bduFile = newBdu;
                        document.getElementById('bduFileName').textContent = bduFile.name;
                    }
                ...
                async function loadConfig() {
                    try {
                        ...
                        const data = await response.json();
                        if (data.status && data.config) {
                            if (data.config.filename_cve) {
                                document.getElementById('cveFileName').textContent = data.config.filename_cve;
                            }
                            if (data.config.filename_bdu) {
                                document.getElementById('bduFileName').textContent = data.config.filename_bdu;
                            }
                        }
                    } catch (error) {
                ...
                document.addEventListener('DOMContentLoaded', function() {
                    ...
                    if (!restored) {
                        // Если нет активной задачи, загружаем конфигурацию
                        loadConfig();
...
async def process_files(
    ...
    try:
        ...
        if load_bdu:
            ...
            task_statuses[task_id]["progress"] = 65
            ...
            if bdu_file_path and os.path.exists(bdu_file_path):
                try:
                    ...
                    task_statuses[task_id]["progress"] = 90
                except Exception as e:
                    results.append({
                        "status": False,
