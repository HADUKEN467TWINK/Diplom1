...
def atr_simple_none(doc: dict, text: str, vul):
    """Обработка простых атрибутов BDU"""
...
async def update_cve_in_mongo(update_url: str):
    ...
    with tempfile.TemporaryDirectory() as temp_file:
        ...
        return {
            "status": True,
            "message": "База данных CVE успешно обновлена!",
            "statistics": {
    ...
    with tempfile.TemporaryDirectory() as temp_file:
        ...
        for elem in vulnerabilities:
            try:
                ...
                errors += 1
        if successful_creates == 0 and successful_updates == 0:
            return {
                "status": False,
        ...
        if excp_change:
            statistics["Предупреждение"] = "Структура файлов БДУ могла быть изменена авторами. Проверьте структуру полей в файлах."
        return {
            "status": True,
            "message": "База данных BDU успешно обновлена!",
