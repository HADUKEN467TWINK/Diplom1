...
async def get_config() -> dict:
    ...
    if not config:
        default_config = {
            "update_url_cve": "https://github.com/CVEProject/cvelistV5/archive/refs/heads/main.zip",
            "update_url_bdu": "https://bdu.fstec.ru/files/documents/vulxml.zip",
            "name_base": "data_base"
        }
        r = await get_redis()
...
async def api_delete_key(key: str):
    valid_keys = ["update_url_cve", "update_url_bdu", "name_base"]
...
async def home():
    ...
    <script>
        ...
        async function clearAll() {
            ...
            const keys = ['update_url_cve', 'update_url_bdu', 'name_base'];
            let successCount = 0;
...
if __name__ == "__main__":
    uvicorn.run("main_config:app", host="0.0.0.0", port=8002, reload=True)